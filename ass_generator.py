...
def generate_ass_file(subtitles: List[Dict], video_path: str, video_width: int, video_height:int,use_speaker_styles:bool=False,enable_trans:bool=False,
    tr_prompt: str = '', tr_model: str = 'Qwen3.5-4B', tr_content='',tr_language: str = '中文', enable_single_tr: bool = False, font="方正准圆_GBK", font_size=72,
    PrimaryColour="&H00FFFFFF",SecondaryColour="&H000000FF", OutlineColour="&H00000000", Bold=0, Italic=0, Underline=0,StrikeOut=0,BackColour="&H80000000",
    ScaleX=100, ScaleY=100, Spacing=0, Angle=0,BorderStyle=1, Outline=2, Shadow=0, Alignment=2, MarginL=10, MarginR=10, MarginV=10,Encoding=0,tr_choice="transformers",
    tr_speech_save=True) -> str:
...
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
    ...
    # 生成样式（支持说话人）
    if use_speaker_styles:
        from speaker_diarization import generate_speaker_styles
        speakers = set(sub["speaker"] for sub in subtitles if "speaker" in sub)
        ass_styles = generate_speaker_styles(speakers)
    else:
        # 基础样式
    ...
    for sub in subtitles:
        ...
        # 选择样式（说话人专属/默认）
            ...
            if enable_trans and not enable_single_tr:
                ...
                if sub["source"] == "ocr":
                    a = trsg(sub['text'], ot2tt=tr_prompt, mp=tr_model, ct=tr_content, tl=tr_language,tr_choice=tr_choice)
                    ass_events.append(f"Dialogue: 0,{format_ts(start_ts)},{format_ts(end_ts)},{style},,0,0,0,,{sub.get('ass_pos', '{\\pos(960,1080)}')}{a}")
                    a = ''
                elif sub["source"] == "asr":
                    b = trsg(sub['text'], ot2tt=tr_prompt, mp=tr_model, ct=tr_content, tl=tr_language,tr_choice=tr_choice)
                    if tr_speech_save:
                        ass_events.append(f"Dialogue: 0,{format_ts(start_ts)},{format_ts(end_ts)},{style},,0,0,0,,{ass_text}\\N{b}")
                    else:
                        ass_events.append(f"Dialogue: 0,{format_ts(start_ts)},{format_ts(end_ts)},{style},,0,0,0,,{b}")
                    b = ''
                elif sub["source"] == "singing":
                    c = trsg(sub['text'], ot2tt=tr_prompt, mp=tr_model, ct=tr_content, tl=tr_language,tr_choice=tr_choice)
    ...
    if enable_trans and not enable_single_tr:
        if a:
            a = tr(a, ot2tt=tr_prompt, mp=tr_model, ct=tr_content, tl=tr_language,tr_choice=tr_choice)
        if b:
            b = tr(b, ot2tt=tr_prompt, mp=tr_model, ct=tr_content, tl=tr_language,tr_choice=tr_choice)
        if c:
            c = tr(c, ot2tt=tr_prompt, mp=tr_model, ct=tr_content, tl=tr_language,tr_choice=tr_choice)
...
def create_subtitle_structure(text_key, seg, fps):
    """构建单个字幕字典的辅助函数"""
    text, _, _ = text_key
    frames = sorted(seg["frames"])
    start_ts = frame_to_timestamp(frames[0], fps)
    end_ts = frame_to_timestamp(frames[-1], fps)
    ass_x = round(seg["x_mid"])
    ass_y = max(0, round(seg["y_top"] - 5))
    ...
    return {
        ...
        "resource":"ocr",
        "time_stamp": f"{start_ts},{end_ts}",
        "ass_pos": f"{{\\pos({ass_x},{ass_y})\\b}}"
    }